...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const codeGeneratorTypeSelect = document.getElementById('codeGeneratorType');
    const languageBindingSelect = document.getElementById('languageBinding');
    const browserEngineSelect = document.getElementById('browserEngine');
    ...
    // Save language binding selection
    if (languageBindingSelect) {
      languageBindingSelect.addEventListener('change', (e) => {
        storage.set({ selectedLanguage: e.target.value });
      });
    }

    // Save browser engine selection
    if (browserEngineSelect) {
      browserEngineSelect.addEventListener('change', (e) => {
        storage.set({ selectedBrowserEngine: e.target.value });
      });
    }

    // Sync checkboxes to match the chosen code generator type
    function applyCheckboxesForGeneratorType(type) {
      ...
      const testsCheckbox   = document.getElementById('javaGenModeTests');
      if (!featureCheckbox || !pageCheckbox || !testsCheckbox) return;

      const checkboxMap = {
        'SELENIUM_JAVA_PAGE_ONLY':                      { feature: false, page: true,  tests: false },
        'CUCUMBER_ONLY':                                { feature: true,  page: false, tests: false },
        'CUCUMBER_WITH_SELENIUM_JAVA_STEPS':            { feature: true,  page: false, tests: true  },
        'SELENIUM_JAVA_ALL':                            { feature: true,  page: true,  tests: true  },
        'PLAYWRIGHT_TYPESCRIPT_PAGE_ONLY':              { feature: false, page: true,  tests: false },
        'PLAYWRIGHT_TYPESCRIPT_WITH_TESTS':             { feature: false, page: true,  tests: true  },
        'PLAYWRIGHT_TYPESCRIPT_FEATURE_ONLY':           { feature: true,  page: false, tests: false },
        'PLAYWRIGHT_TYPESCRIPT_WITH_FEATURE_AND_TESTS': { feature: true,  page: false, tests: true  },
        'PLAYWRIGHT_TYPESCRIPT_ALL':                    { feature: true,  page: true,  tests: true  },
      };

      const state = checkboxMap[type];
      if (state) {
        featureCheckbox.checked = state.feature;
        pageCheckbox.checked    = state.page;
        testsCheckbox.checked   = state.tests;
      }
    }

    // Handle code generator type selection
    if (codeGeneratorTypeSelect) {
      codeGeneratorTypeSelect.addEventListener('change', (e) => {
        const selectedType = e.target.value;
        storage.set({ codeGeneratorType: selectedType });
        applyCheckboxesForGeneratorType(selectedType);

        // If Playwright is selected, automatically set Language to TypeScript and Browser Engine to Playwright
        if (selectedType.includes('PLAYWRIGHT')) {
          if (languageBindingSelect) {
            languageBindingSelect.value = 'ts';
            storage.set({ selectedLanguage: 'ts' });
          }
          if (browserEngineSelect) {
            browserEngineSelect.value = 'playwright';
            storage.set({ selectedBrowserEngine: 'playwright' });
          }
        }
        // If Selenium or Cucumber is selected, automatically set Language to Java and Browser Engine to Selenium
        else if (selectedType.includes('SELENIUM') || selectedType.includes('CUCUMBER')) {
          if (languageBindingSelect) {
            languageBindingSelect.value = 'java';
            storage.set({ selectedLanguage: 'java' });
          }
          if (browserEngineSelect) {
            browserEngineSelect.value = 'selenium';
            storage.set({ selectedBrowserEngine: 'selenium' });
          }
        }
      });
    }
    ...
    const result = await new Promise(resolve => {
      storage.get(['groqApiKey', 'openaiApiKey','testleafApiKey', 'selectedModel', 'selectedProvider', 'codeGeneratorType', 'selectedLanguage', 'selectedBrowserEngine'], resolve);
    });
    ...
    if (result.codeGeneratorType && codeGeneratorTypeSelect) {
      codeGeneratorTypeSelect.value = result.codeGeneratorType;
      applyCheckboxesForGeneratorType(result.codeGeneratorType);
    } else if (codeGeneratorTypeSelect) {
      // Default to Selenium Java - Page Object Only
      codeGeneratorTypeSelect.value = 'SELENIUM_JAVA_PAGE_ONLY';
      storage.set({ codeGeneratorType: 'SELENIUM_JAVA_PAGE_ONLY' });
      applyCheckboxesForGeneratorType('SELENIUM_JAVA_PAGE_ONLY');
    }
    if (result.selectedLanguage && languageBindingSelect) {
      languageBindingSelect.value = result.selectedLanguage;
    } else if (languageBindingSelect) {
      // Default to Java
      languageBindingSelect.value = 'java';
      storage.set({ selectedLanguage: 'java' });
    }
    if (result.selectedBrowserEngine && browserEngineSelect) {
      browserEngineSelect.value = result.selectedBrowserEngine;
    } else if (browserEngineSelect) {
      // Default to Selenium
      browserEngineSelect.value = 'selenium';
      storage.set({ selectedBrowserEngine: 'selenium' });
    }
