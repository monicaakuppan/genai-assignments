...
class ChatUI {
    ...
    initialize() {
        ...
        chrome.storage.onChanged.addListener((changes) => {
            ...
            if (changes.codeGeneratorType) this.codeGeneratorType = changes.codeGeneratorType.newValue;
        });
    ...
    async sendMessage() {
        ...
        try {
            ...
            // Use codeGeneratorType if set, otherwise fall back to prompt keys based on language/engine
            let promptKeys = [];
            if (this.codeGeneratorType && this.codeGeneratorType !== 'SELENIUM_JAVA_PAGE_ONLY') {
                promptKeys = [this.codeGeneratorType];
            } else {
                promptKeys = this.getPromptKeys(lang, eng);
            }
    ...
    getPromptKeys(language, engine) {
        ...
        const isTestsChecked = checkboxes.some(box => box.value === 'TESTS');

        // Validate that at least one option is selected
        if (!isFeatureChecked && !isPageChecked && !isTestsChecked) {
        ...
        if (isFeatureChecked && isPageChecked && isTestsChecked) {
            // All three selected - generate feature + page object + step definitions
            if (this.isJavaSelenium(lang, eng)) {
                promptKeys.push('SELENIUM_JAVA_ALL');
            } else if (this.isTypescriptPlaywright(lang, eng)) {
                promptKeys.push('PLAYWRIGHT_TYPESCRIPT_ALL');
            } else {
                promptKeys.push('CUCUMBER_ONLY');
                this.addUnsupportedLanguageMessage(lang, eng);
            }
        } else if (isPageChecked && isTestsChecked) {
            // Page and tests selected - generate page object with tests
            if (this.isJavaSelenium(lang, eng)) {
                promptKeys.push('SELENIUM_JAVA_PAGE_ONLY');
                // Tests generation is implicit for Selenium Java
            } else if (this.isTypescriptPlaywright(lang, eng)) {
            ...
            if (this.isJavaSelenium(lang, eng)) {
                ...
                promptKeys.push('PLAYWRIGHT_TYPESCRIPT_WITH_FEATURE_AND_TESTS');
            } else {
                promptKeys.push('CUCUMBER_ONLY');
                this.addUnsupportedLanguageMessage(lang, eng);
            }
        } else if (isFeatureChecked && isTestsChecked) {
            // Feature and tests selected - generate feature file with step definitions
            if (this.isJavaSelenium(lang, eng)) {
                promptKeys.push('CUCUMBER_WITH_SELENIUM_JAVA_STEPS');
            } else if (this.isTypescriptPlaywright(lang, eng)) {
                promptKeys.push('PLAYWRIGHT_TYPESCRIPT_WITH_FEATURE_AND_TESTS');
            ...
            if (this.isJavaSelenium(lang, eng)) {
                ...
                promptKeys.push('PLAYWRIGHT_TYPESCRIPT_FEATURE_ONLY');
            } else {
                promptKeys.push('CUCUMBER_ONLY');
                this.addUnsupportedLanguageMessage(lang, eng);
            }
        ...
        } else if (isTestsChecked) {
            // Tests only
            if (this.isTypescriptPlaywright(lang, eng)) {
                promptKeys.push('PLAYWRIGHT_TYPESCRIPT_WITH_TESTS');
            } else {
                this.addMessage('Test generation is only available with TypeScript/Playwright combination.', 'system');
            }
        }
    ...
    addUnsupportedLanguageMessage(language, engine) {
        const message = `⚠️ ${language}/${engine} combination is not yet supported. Supported combinations: Java/Selenium, TypeScript/Playwright.`;
